...
# Here we feed text into the prompt, right before our question. No Retrieval is taking place.
...
def fast_start_prompting(model_name):
    ...
    for idx, entry in enumerate(test_list):
        
        print(f'\n{idx+1}. Query: {entry["query"]}')

        # output is a dict:
        # key 'llm_response' -> value is output from the model
        # key 'usage' -> value is another dict with usage stats

        output = prompter.prompt_main(	entry['query'],
        ...
        print(f'Gold answer: {entry["answer"]}')
...
if __name__ == '__main__':
    ...
    #
    #for model_set in [llm_models, llm_local_models, llm_open_source_models]:
    #    print(f'----------------')
    #    for idx, model in enumerate(model_set):
    #        print(f'model {idx}:', model['model_name'], model['model_family'])
    #
    ## Below are some good models to start with.    
    #model_short_list = ["llmware/bling-1b-0.1",
    #                    "llmware/bling-tiny-llama-v0",
    #                    "llmware/dragon-yi-6b-gguf",
    #                    "llmware/bling-falcon-1b-0.1"] 
    
    model_name = 'llmware/bling-1b-0.1'				# Small model fine-tuned for RAG, 1000 times smaller than GPT-4.

    # To swap to using GPT-4 with API calls vs. using a locally running model, uncomment this:
    # model_name = 'gpt-4'
    # os.environ['USER_MANAGED_OPENAI_API_KEY'] = '<insert key>'
